...
/**
 * Fire-and-forget POST /onboarding/message so the backend LLM can extract
 * profile fields via function-calling. We don't use the LLM's reply as the
 * next assistant message (the scripted flow keeps the UX predictable); the
 * backend just silently updates user_profiles as it learns from each answer.
 */
async function pushOnboardingAnswer(stepId, displayText) {
  if (isDemoMode() || !displayText) return;
  try {
    await api.post('/onboarding/message', {
      message: `[${stepId}] ${displayText}`,
    });
  } catch { /* non-fatal */ }
}
...
export default function OnboardingChatScreen({
  ...
  const goNext = async (displayText, value) => {
    ...
    persist(nextAnswers);
    // Send answer to backend LLM for incremental profile extraction (non-blocking).
    if (value != null) pushOnboardingAnswer(current.id, String(displayText ?? value));
          ...
          {current.inputType === 'chips' ? null : (
            ...
            >
              <input
                ...
                onKeyDown={onKeyDown}
                aria-label="Onboarding response"
                placeholder={
                  current.placeholder
                    ? current.placeholder
