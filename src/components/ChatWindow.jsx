...
import AISummaryModal from './chat/AISummaryModal';
import AIAssistPanel from './chat/AIAssistPanel';
import AddInvoiceModal from './chat/AddInvoiceModal';
...
export default function ChatWindow({ project, meetingInviteOpen, onMeetingInviteOpenChange, onProjectStatusChanged, onBackToList, onMessageActivity }) {
  const navigate = useNavigate();
  const { user } = useAuth();
  const { openTimerPanel, closeTimerPanel, timerPanelOpen } = useTimeTracking();
  const { markProjectRead, refresh: refreshUnread } = useUnreadMessages();
  ...
  const handleAttachmentSelected = async (e) => {
    ...
    if (isDemoMode()) {
      setUploadError('File uploads require a running backend (not available in demo mode).');
    ...
    try {
      ...
      await api.post(`/projects/${project.id}/messages/attachment`, form, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      setInput('');
      // The backend broadcasts the message via WebSocket, so it'll appear in `messages` automatically.
    } catch (err) {
  ...
  // Mark project as read whenever the user opens this chat
  useEffect(() => {
    if (!project?.id || isDemoMode()) return;
    markProjectRead(project.id);
  }, [project?.id, markProjectRead]);

  // When a new message lands, refresh other users' unread counts (and mark as read for this user)
  useEffect(() => {
    if (!project?.id || isDemoMode()) return;
    if (!messages.length) return;
    markProjectRead(project.id);
    refreshUnread();
  }, [messages.length, project?.id, markProjectRead, refreshUnread]);

  // Bubble the latest message timestamp/text up so the thread list can re-sort live.
  useEffect(() => {
    if (!project?.id || !messages.length || !onMessageActivity) return;
    const last = messages[messages.length - 1];
    if (!last) return;
    onMessageActivity(project.id, last.text ?? '', last.created_at ?? new Date().toISOString());
  }, [messages, project?.id, onMessageActivity]);
  ...
  useEffect(() => {
    ...
    if (!isDemoMode()) {
      ...
      setScopeAlerts([]);
      // Load decisions from backend (best-effort)
      (async () => {
        try {
          const { api } = await import('../api/client');
          const { data } = await api.get(`/projects/${project.id}/decisions`);
          if (Array.isArray(data) && data.length > 0) {
            ...
            setDecisions((prev) => {
              ...
              return [...mapped.filter((m) => !existing.has(m.id)), ...prev];
            });
          }
        } catch { /* noop */ }
      })();
  ...
  const logMessageAsDecision = async (msg) => {
    setContextMenu(null);
    setDecisionLogOpen(true);
    const summary = typeof msg.text === 'string' ? msg.text.slice(0, 200) : 'Logged from chat';
    const tempId = `dec-${Date.now()}`;
    setDecisions((prev) => [
      ...prev,
      {
        id: tempId, at: new Date().toISOString(),
        who: msg.sender_name || 'Participant', summary, tag: 'Other', source: 'chat',
      },
    ]);
    if (!isDemoMode() && project?.id) {
      try {
        const { api } = await import('../api/client');
        const msgIdNum = typeof msg.id === 'number' ? msg.id : null;
        await api.post(`/projects/${project.id}/decisions`, {
          decision: summary, message_id: msgIdNum, tag: 'Decision',
        });
  ...
  const addDecision = async (entry) => {
    ...
    if (!isDemoMode() && project?.id) {
      try {
        const { api } = await import('../api/client');
        await api.post(`/projects/${project.id}/decisions`, {
          decision: entry.summary || entry.text || 'Decision',
          tag: entry.tag || 'Decision',
        });
      } catch { /* noop */ }
    }
  };
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0, minWidth: 0, width: '100%' }}>
      ...
      >
        {/* Back arrow — on mobile this returns to the thread list, on desktop to the dashboard */}
        <button
          type="button"
          onClick={() => {
            if (isMobile && onBackToList) onBackToList();
            else navigate('/');
          }}
          style={{
            width: 36,
          ...
          aria-label={isMobile ? 'Back to thread list' : 'Back to dashboard'}
        >
        ...
        <div style={{ flex: 1, textAlign: 'center', minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8, marginTop: 4, flexWrap: 'wrap' }}>
            ...
            <span style={{ fontSize: 11, color: 'var(--color-text-3)', display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              />
              <span className="scout-chat-conn-text">{headerStatus}</span>
            </span>
          </div>
        </div>

        {/* Right side: AI buttons + video camera + three-dot menu.
            On mobile we drop the standalone AI buttons entirely — they're duplicated
            inside the 3-dot menu, so the header has room for the project title. */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
          {!isMobile && (
            <button
              type="button"
              onClick={() => setAISummaryOpen(true)}
              title="AI Summary — summarize this chat"
              className="scout-chat-ai-btn"
              style={{
            ...
            >
              ...
              <span className="scout-ai-btn-label">Summary</span>
            </button>
          )}
          {!isMobile && (
            <button
              ...
              title="AI Assist — ask anything about this chat"
              className="scout-chat-ai-btn"
              style={{
                display: 'inline-flex', alignItems: 'center', gap: 6,
                height: 40, padding: '0 12px',
            ...
            >
              ...
              <span className="scout-ai-btn-label">Ask AI</span>
            </button>
          )}
      ...
      <div style={{ padding: '12px 16px', borderTop: '1px solid var(--color-border)', background: 'var(--color-surface)', flexShrink: 0 }}>
        ...
        >
          ...
          <button
            ...
            className="scout-chat-invoice-btn"
            style={{
              height: 36,
          ...
          >
            ...
            {!isMobile && <span className="scout-chat-invoice-label">Invoice</span>}
          </button>
          <button
            type="button"
            onClick={handleSend}
        ...
        {uploadError ? (
          <p style={{ fontSize: 12, color: 'var(--color-danger)', marginTop: 8, lineHeight: 1.45 }}>{uploadError}</p>
        ) : null}
        {uploadingAttachment ? (
          <p style={{ fontSize: 12, color: 'var(--color-text-3)', marginTop: 8, lineHeight: 1.45 }}>Uploading…</p>
        ) : null}
      ...
      {showAddInvoice && project?.id ? (
        <AddInvoiceModal
          projectId={project.id}
          projectName={project.name}
          onClose={() => setShowAddInvoice(false)}
          onCreated={() => {
            // refresh chat so the invoice message shows up (it's already inserted server-side)
            try {
              window.dispatchEvent(new CustomEvent('scout-toast', {
                detail: { type: 'success', message: 'Invoice submitted.' },
              }));
            } catch { /* noop */ }
          }}
        />
      ...
      <button
        ...
        title="Time tracker"
        style={{
          /* Anchor to LEFT side so it never overlaps the send button on the right */
          position: 'fixed',
          left: isMobile ? 16 : undefined,
          right: isMobile ? undefined : 24,
          /* Sit ABOVE the chat input area: input is ~100px tall + hint; lift clear */
          bottom: isMobile ? 160 : 140,
          width: 48,
          height: 48,
          borderRadius: '50%',
          border: 'none',
          ...
          alignItems: 'center',
          justifyContent: 'center',
          boxShadow: '0 6px 18px rgba(29, 110, 205, 0.3)',
