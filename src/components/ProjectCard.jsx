...
export default function ProjectCard({ project, isBookmarked, onToggleBookmark, unreadCount }) {
  ...
  const liveUnread = typeof unreadCount === 'number' ? unreadCount : project.unread_count;
  ...
  return (
    <article
      style={{
        border: hasUnread ? '1px solid var(--color-primary)' : '1px solid var(--color-border)',
        borderRadius: 14,
        ...
        gap: 12,
        boxShadow: hasUnread ? '0 0 0 3px rgba(29, 110, 205, 0.12)' : '0 1px 0 rgba(15,22,35,0.04)',
        position: 'relative',
    ...
    >
      {hasUnread && (
        <span
          aria-label={`${liveUnread} unread message${liveUnread === 1 ? '' : 's'}`}
          style={{
            position: 'absolute',
            top: -8,
            right: -8,
            ...
            lineHeight: 1,
            boxShadow: '0 2px 6px rgba(220, 38, 38, 0.4), 0 0 0 3px var(--color-surface)',
            zIndex: 1,
          }}
        >
